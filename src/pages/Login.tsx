  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-gradient-to-br from-primary/5 via-background to-secondary/5 overflow-hidden">
      ...
      <Header 
        ...
        onToggleHeader={() => {}}
      />
      <div className="flex-1 flex items-center justify-center p-4 xs-mobile:p-2 sm-mobile:p-3">
        <Card className="responsive-card w-full max-w-md p-6 sm-mobile:p-4 tablet:p-8 space-y-6 shadow-xl">
          <div className="text-center space-y-2">
            <div className="flex justify-center">
              <img
                src={oxfordIgniteLogo}
                alt="Oxford Ignite"
                className="h-16 w-auto xs-mobile:h-12 sm-mobile:h-14 tablet:h-18 lg-tablet:h-20"
              />
            </div>
          </div>

          <form onSubmit={handleLogin} className={`responsive-form space-y-4 xs-mobile:space-y-3 ${isLoading ? 'opacity-70' : ''}`}>
            <div className="space-y-2 xs-mobile:space-y-1">
              <Label htmlFor="username" className="text-sm xs-mobile:text-xs">Username</Label>
              <Input
                id="username"
                type="text"
            ...
            <div className="space-y-2 xs-mobile:space-y-1">
              <Label htmlFor="password" className="text-sm xs-mobile:text-xs">Password</Label>
              <div className="relative">
                <Input
                  ...
                  className="xs-mobile:min-h-[2.5rem] sm-mobile:min-h-[2.75rem] pr-10"
                  required
                />
                <button
                  type="button"
                  className="absolute right-3 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground xs-mobile:right-2 xs-mobile:p-1"
                  onClick={() => setShowPassword(!showPassword)}
                  aria-label={showPassword ? "Hide password" : "Show password"}
                >
                  {showPassword ? (
                    <EyeOff className="h-4 w-4 xs-mobile:h-3 xs-mobile:w-3" />
                  ) : (
                    <Eye className="h-4 w-4 xs-mobile:h-3 xs-mobile:w-3" />
                  )}
                </button>
              </div>
            </div>
            ...
            <Button 
              type="submit" 
              className="responsive-button w-full xs-mobile:min-h-[2.5rem] sm-mobile:min-h-[2.75rem]" 
              disabled={isLoading}
            >
              {isLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 xs-mobile:h-3 xs-mobile:w-3 animate-spin" />
                  <span className="xs-mobile:text-sm">Signing in...</span>
                </>
              ) : (
                <span className="xs-mobile:text-sm">Sign In</span>
              )}
            </Button>
            
            <div className="flex justify-between xs-mobile:flex-col xs-mobile:gap-2 sm-mobile:flex-row sm-mobile:gap-4">
              <a 
                href="#"
                className="inline-flex items-center justify-start whitespace-nowrap text-sm font-medium focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 h-8 px-0 py-2 xs-mobile:text-xs xs-mobile:h-6 xs-mobile:justify-center"
                onClick={(e) => {
                  e.preventDefault();
              ...
              <a 
                ...
                className="inline-flex items-center justify-end whitespace-nowrap text-sm font-medium focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 h-8 px-0 py-2 xs-mobile:text-xs xs-mobile:h-6 xs-mobile:justify-center"
              >
                Admin login
              </a>
            </div>
