(function (global) {
    ...
    function TextHighlighter(element, options) {
        ...
        this.options = defaults(options, {
            ...
            onAfterHighlight: function () {return true; }
        });
    ...
    TextHighlighter.prototype.storeRange = function () {
    ...
    TextHighlighter.prototype.doHighlight = function (keepRange) {
        ...
        // if (this.options.onBeforeHighlight(range) === true) {
            ...
            //timestamp = timestamp.getMinutes() +""+timestamp.getSeconds();
            timestamp = timestamp.substr(timestamp.length - 4);
            //console.log(timestamp);
            wrapper = TextHighlighter.createWrapper(this.options);
            wrapper.setAttribute(TIMESTAMP_ATTR, timestamp);   
            
            LastTimeStampUsed = timestamp;
            //console.log("LastTimeStampUsed : ::::: " + LastTimeStampUsed);
        ...
        do {
            if (goDeeper && node.nodeType === NODE_TYPE.TEXT_NODE) {
                ...
                if (IGNORE_TAGS.indexOf(node.parentNode.tagName) === -1 && node.nodeValue.trim() !== '') {
                    ...
                    nodeParent.setAttribute(IS_HIGH, true);
                    //nodeParent.setAttribute(ORG_HTML, "");
                    
                    
                    //console.log(nodeParent.id);
                    if(nodeParent.id == ''){
                        wrapperClone.setAttribute(PARENT_ID, "" + nodeParent.parentNode.id + "");
                        //GetOriginalHTML(nodeParent.parentNode.id);
                    }
                    else{
                        wrapperClone.setAttribute(PARENT_ID, "" + nodeParent.id + "");
                        //GetOriginalHTML(nodeParent.id);
                    }
    ...
    TextHighlighter.prototype.getStoreRange = function () {
        return STORE_RANGE;     
    };   
    ...
    TextHighlighter.createWrapper = function (options) {
        ...
        span.style.backgroundColor = CurrHighData.Clr; //options.color;
    ...
    global.TextHighlighter2 = TextHighlighter;
})(window);