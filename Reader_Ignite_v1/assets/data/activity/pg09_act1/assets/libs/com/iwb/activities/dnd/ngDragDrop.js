...
service('ngDraggable', [function() {
    ...
    scope.inputEvent = function(event) {
        return angular.isDefined(event.originalEvent.touches) ? event.originalEvent.touches[0] : event;
    };
}]).
directive('ngDrag', ['$rootScope', '$parse', '$document', '$window', 'ngDraggable', function ($rootScope, $parse, $document, $window, ngDraggable) {
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            var onDragReleaseCallback = $parse(attrs.ngDragRelease) || null;
            ...
            var onrelease = function(evt) {
                ...
                onDragReleaseCallback
                if (!onDragReleaseCallback )return;

                scope.$apply(function () {
                    onDragReleaseCallback(scope, {$data: _data, $event: evt});
                });
            };
                ...
                if (isTouching(obj.x, obj.y, obj.element)) {
                   ...
                   if(obj.callback){
                    ...
                    if (attrs.ngDropSuccess) {
                        $timeout(function(){
                            onDropCallback(scope, {$data: obj.data, $event: obj, $targetID: scope.dropID, $target: scope.$eval(scope.value)});
                        });
                    }
                }
